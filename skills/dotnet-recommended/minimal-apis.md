...
In .NET 10, the WithOpenApi() extension methods on minimal API route builders are deprecated because their functionality is now redundant with the built-in OpenAPI document generation pipeline provided by the Microsoft.AspNetCore.OpenApi package. Calling .WithOpenApi() on endpoints still compiles but emits a deprecation warning (ASPDEPR002) and should be removed. Instead of marking endpoints individually with WithOpenApi(), you simply register OpenAPI generation globally (e.g., builder.Services.AddOpenApi() and app.MapOpenApi() for the spec endpoint) and rely on metadata from route signatures, attributes (like [Tags], [Produces], etc.), and OpenAPI document/operation transformers such as AddOpenApiOperationTransformer to customize the OpenAPI spec. This centralizes and modernizes how Minimal APIs surface OpenAPI metadata without per-endpoint WithOpenApi() calls.
