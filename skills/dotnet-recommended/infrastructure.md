...
### High-performance logging (preferred)

For high-performance structured logging in .NET (especially in production code with many log calls), prefer the compile-time logging source generator pattern using [LoggerMessage] on partial methods instead of dozens of naïve logger.Log…() calls. This lets the C# source generator produce optimized logging code that eliminates boxing, reduces allocations, and parses message templates at compile time rather than at every runtime call, giving much better performance with minimal overhead. To follow this pattern, define your log methods as partial in a static helper class (or in the class itself) and decorate them with LoggerMessage specifying EventId, Level, and Message; then call these generated helpers from your application code just like normal logging. For example:

```csharp
internal static partial class Log
{
    [LoggerMessage(Level = LogLevel.Information, Message = "Processing order {OrderId} for customer {CustomerId}.")]
    public static partial void OrderProcessed(ILogger logger, int orderId, int customerId);
}
// usage
Log.OrderProcessed(logger, order.Id, order.CustomerId);
```

This gives you maintainable, structured logs with significantly lower overhead than regular ILogger.LogInformation in hot paths (https://learn.microsoft.com/en-us/dotnet/core/extensions/logger-message-generator).
